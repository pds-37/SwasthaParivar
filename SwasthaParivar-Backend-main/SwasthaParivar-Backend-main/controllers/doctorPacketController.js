...
const serializePacket = (packet) => {
  ...
  return {
    ...
    randomForestRisk: normalized.randomForestRisk || "NOT_EVALUATED",
    logisticProbability: typeof normalized.logisticProbability === "number" ? normalized.logisticProbability : 0.0,
    decisionTreeReasoning: normalized.decisionTreeReasoning || [],
    clinicalRulesVerdict: normalized.clinicalRulesVerdict || "NOT_APPLICABLE",
    geminiSummary: normalized.geminiSummary || normalized.summary || "",
...
const buildExportText = (packet) => {
  ...
  const sections = [
    ...
    [
      "AI Machine Learning Diagnostic & Explainability",
      [
        `Random Forest Prediction: ${data.randomForestRisk}`,
        `Calibrated Probability (LogReg): ${(Number(data.logisticProbability || 0) * 100).toFixed(1)}%`,
        `Clinical Rules Verdict: ${data.clinicalRulesVerdict}`,
        `Decision Tree Rule Path: ${Array.isArray(data.decisionTreeReasoning) && data.decisionTreeReasoning.length ? data.decisionTreeReasoning.join(" => ") : "None extracted"}`,
      ],
    ],
    ["Gemini Clinical Narrative Summary", [data.geminiSummary || data.summary]],
...
export const createDoctorPacket = async (req, res) => {
  ...
  const packet = await DoctorPacket.create({
    ...
    randomForestRisk: req.body.randomForestRisk || "NOT_EVALUATED",
    logisticProbability: typeof req.body.logisticProbability === "number" ? req.body.logisticProbability : 0.0,
    decisionTreeReasoning: normalizeList(req.body.decisionTreeReasoning),
    clinicalRulesVerdict: req.body.clinicalRulesVerdict || "NOT_APPLICABLE",
    geminiSummary: req.body.geminiSummary || req.body.summary || "",
